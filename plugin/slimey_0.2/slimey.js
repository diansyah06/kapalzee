...
var Slimey = function(config) {
	...
	if (config.idDock) this.idDock = config.idDock;
	if (config.idproje) this.idproje = config.idproje;
...
Slimey.prototype.submitFile = function() {
	...
	var idKegiatans= this.idproje;
	var iddocument =this.idDock;
	var comment = prompt("Please leave the comment");

	if (comment != null) {
		$.post("process.php", {ext:ext, dataFile:sc.value, idKegiatansnnn:idKegiatans, act: act , modul: modul, idobject:iddocument,comment:comment } , function(html) {
			$('.revison').html(html);
			$(".revison").hide();
			$(".revison").fadeIn(400);});
	...
	var subscr=$("#listsubscriber", window.parent.document).val();
	var modul= "documentproj";
	var act= "WriteDocument";
	var iddocument =0;
	var idKegiatans=$("#idKegiatans", window.parent.document).val();
	
			$.post("../../modern/process.php", {ext:ext, dataFile:sc.value, idKegiatans:idKegiatans, act: act , modul: modul, iddocument:iddocument,nama:fn.value ,subscr:subscr} , function(html) {
			$('.document', window.parent.document).html(html);
			$(".document", window.parent.document).hide();
			$(".document", window.parent.document).fadeIn(400);}); 
	parent.slideAddSlime();	
	}
...
Slimey.includeScripts = function() {
	...
	document.write('<script language="javascript" src="' + 'https://ajax.googleapis.com/ajax/libs/jquery/2.0.3/jquery.min.js"></script>');

}
Slimey.loadJquery = function() {
...
Slimey.preloadImages = function() {
	...
	Slimey.preloadToolbarImage('center');
	Slimey.preloadToolbarImage('left');
	Slimey.preloadToolbarImage('right');
